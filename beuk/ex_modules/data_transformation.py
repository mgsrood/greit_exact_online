...
def apply_appending_functions(token_struct, df, tabel, klantnaam, finn_it_connection_string, script_id, script, division_code, division_name):
    
    # Start log
    log(token_struct, finn_it_connection_string, klantnaam, f"Start van data transformatie", script_id, script, division_code, tabel)
    ...
    try:
        if tabel == "Verkoopfacturen":
            ...
            log(token_struct, finn_it_connection_string, klantnaam, f"Onbekend tabel type, geen actie uitgevoerd", script_id, script, division_code, tabel)
            return df

        # Succes log
        log(token_struct, finn_it_connection_string, klantnaam, f"Data transformatie gelukt", script_id, script, division_code, tabel)

    except Exception as e:
        # Foutmelding log
        print(f"FOUTMELDING | Fout bij het transformeren van {tabel} | {division_name} ({division_code}) | {klantnaam}")
        log(token_struct, finn_it_connection_string, klantnaam, f"FOUTMELDING | Fout bij het transformeren: {e}", script_id, script, division_code, tabel)
        print(e)
        return df
