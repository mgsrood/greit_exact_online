        ...
        if column in df.columns:
            try:
                if dtype == 'uniqueidentifier':
                    ...
                    # Zorg ervoor dat de kolom eerst als string wordt behandeld
                    df[column] = df[column].astype(str).str.lower().map({'true': True, 'false': False, '1': True, '0': False})
...
def apply_type_conversion(token_struct, df, finn_it_connection_string, klantnaam, script_id, script, division_code, tabel):
    
    log(token_struct, finn_it_connection_string, klantnaam, f"Start data type conversie", script_id, script, division_code, tabel)
    ...
    if column_types is None:
        ...
        log(token_struct, finn_it_connection_string, klantnaam, f"FOUTMELDING | Geen data type mapping gevonden", script_id, script, division_code, tabel)
        return None
    ...
    # Succes en start log
    log(token_struct, finn_it_connection_string, klantnaam, f"Data type conversie succesvol", script_id, script, division_code, tabel)
