...
class ExactFinancialsError(RuntimeError):
    pass

class ExactFinancials:
    ...
    def intial_get_administraties(self):
        spark = SparkSession.builder.getOrCreate()
        ...
        if response.status_code == 200:
            data = response.json()
            df = spark.createDataFrame(data['admat'])
            return df
        else:
            logging.error(f"Fout bij het ophalen van de administraties: {response.status_code}")
            raise ExactFinancialsError(f"Fout bij het ophalen van de administraties: {response.status_code}")
