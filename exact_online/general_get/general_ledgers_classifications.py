...
if __name__ == "__main__":
    ...
    endpoint = "/bulk/Financial/GLClassifications?$select=ID,Abstract,Balance,Code,Created,Creator,CreatorFullName,Description,Division,IsTupleSubElement,Modified,Modifier,ModifierFullName,Name,Nillable,Parent,PeriodType,SubstitutionGroup,TaxonomyNamespace,TaxonomyNamespaceDescription,Type"

    df = get_request(division_code, url, endpoint)
    if df is not None:
        print(df)
        export_to_csv(df, "general_ledgers_classifications.csv")
