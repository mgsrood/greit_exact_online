...
if __name__ == "__main__":
    ...
    for klantnaam, connection_string in connection_dict.items():
        ...
        for division_name, division_code in division_dict.items():
            ...
            endpoints = {"ReportingBalance": f"financial/ReportingBalance?$filter=ReportingYear ge {reporting_year}&$select=ID,Amount,AmountCredit,AmountDebit,BalanceType,CostCenterDescription,CostUnitDescription,Count,Division,GLAccount,ReportingPeriod,ReportingYear"}
            ...
            for tabel, endpoint in endpoints.items():
                ...
                # Clear the table
                clear_table(connection_string, tabel, table_mode, reporting_year)
...
Openstaand = {
    "RelatieKeten": f"crm/AccountClassifications?$filter=Modified ge datetime'{laatste_sync}'&$select=AccountClassificationName,AccountClassificationNameDescription,Code,Description,Division",
    "GrootboekRubriek": f"bulk/Financial/GLClassifications?$filter=Modified ge datetime'{laatste_sync}'&$select=Code,Description,Division,ID,Parent,TaxonomyNamespaceDescription",
}